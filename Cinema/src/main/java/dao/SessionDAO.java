...
public class SessionDAO extends DAO {
    ...
    private static final String ID_SESSION = "WHERE f.film_id = ? AND s.room_id = ? AND s.date = ? AND s.time = ?";    
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Session session = new Session(rs.getInt("session_id"),
                        rs.getBigDecimal("ticket_price"),
                        rs.getObject("date", LocalDate.class),
                        rs.getObject("date", LocalTime.class),
                        film,
                        rs.getInt("free_seats"));
                session.setRoom(room);
                return session;
    ...
    public int getIdSession(int filmid,int roomid, LocalDate date, LocalTime time){
        String query = ALL_SESSION + ID_SESSION ;
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
            ps.setInt(1, filmid);
